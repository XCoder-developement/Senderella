...
class ChatResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'date' => optional($last_message->created_at)->format('Y-m-d') ?? '',
            'time' => optional($last_message->created_at)->format('g:i A') ?? '',
            'sender_id' => $last_message->user_id ?? '',
            'new_message_count' => $this->messages()->where('is_read', 0)->where('user_id', '!=', auth()->id())->count(),
            'partner' => new PartnerResource(User::find($request->receiver_id)),
